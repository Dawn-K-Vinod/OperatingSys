...
void RoundRobin(Process pcopy[], int np) {
    ...
    while (completed < np) {
        ...
        int all_done_or_not_arrived = 1;
        for (int j = 0; j < np; j++) {
            if (pcopy[j].remaining > 0 && pcopy[j].AT <= time) {
                all_done_or_not_arrived = 0;
                break;
            }
        }
        if (all_done_or_not_arrived) {
            time++;
            continue;///////////
        }
        
        // move in circular manner (THIS is your "implicit queue")
        i = (i + 1) % np;
    }
    rr_avg_wt /= np;
    display(pcopy, np);
